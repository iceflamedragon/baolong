...
Size sizeSquare =
    Size(20, 20); // 实际测量标定板上每个棋盘格的边长（长,宽）：单位mm
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < imagesPath.size(); i++) {
    ...
    if (!getImageSize)                      // 初始化图像Size信息
    {
      ...
      getImageSize = true;
      cout << "Image.width = " << sizeImage.width
           << "  |  Image.height = " << sizeImage.height << endl;
    }

    vector<Point2f> pointCorners; // 每幅图像上检测到的角点坐标
    if (findChessboardCorners(imageInput, sizeBoard,
                              pointCorners)) // 寻找图像角点
    {
      Mat imageGray;
      cvtColor(imageInput, imageGray, 7);
      //[2] 亚像素精确化
      ...
      drawChessboardCorners(imageInput, sizeBoard, pointCorners,
      ...
      uint16_t countCorner =
          sizeBoard.width * sizeBoard.height; // 每张图像上该有的角点数量
  ...
  calibrateCamera(pointsObject, pointsCorners, sizeImage, cameraMatrix,
  ...
  cout << "总体平均误差: " << errorTotal / pointsCorners.size() << "像素"
       << endl;
  fout << "总体平均误差: " << errorTotal / pointsCorners.size() << "像素"
       << endl
       << endl;
...
void imageCorrecte(string imagesPath) {
  ...
  Mat imageCorrect = imageSource.clone();
  // 采用initUndistortRectifyMap+remap进行图像矫正
  initUndistortRectifyMap(matrix, coeffs, R, matrix, sizeImage, CV_32FC1, mapx,
                          mapy);
  remap(imageSource, imageCorrect, mapx, mapy, INTER_LINEAR);

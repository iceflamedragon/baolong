...
void PID_init() {
  ...
  Speed_left.ki_ratio =
      0.1; // 增量式0.1,位置式0.01//纯速度环0.1，电流环串速度环0.0001
...
void PID_struct_init(PID_STRUCT *PID, float kp_ratio, float ki_ratio,
                     float kd_ratio, float integral_Limit,
...
float PID_Positional(PID_STRUCT *PID, float present_value, float target_value) {
  ...
  PID->PID_out_P = (PID->PID_para->kp) * (PID->kp_ratio) *
                   PID->value_err[0]; // 计算PID中的KP分量
  PID->PID_out_I += (PID->PID_para->ki) * (PID->ki_ratio) *
                    (PID->value_err[0]); // 计算PID中的KI分量
  PID->PID_out_D = (PID->PID_para->kd) * (PID->kd_ratio) *
                   (PID->value_err[0] - PID->value_err[1]); // 计算PID中的KD分量
  if (PID->PID_out_I > PID->integral_Limit)
    PID->PID_out_I = PID->integral_Limit; // 积分限幅
  else if (PID->PID_out_I < -PID->integral_Limit)
    PID->PID_out_I = -PID->integral_Limit;
  ...
  if (PID->PID_output > PID->total_limit)
    PID->PID_output = PID->total_limit;
  else if (PID->PID_output < -PID->total_limit)
    PID->PID_output = -PID->total_limit;
  return PID->PID_output;
}

// 增量式PID
float PID_Incremental(PID_STRUCT *PID, float present_value,
                      float target_value) {
  PID->target_value = target_value;   // 获取目标变量值
  PID->present_value = present_value; // 获取当前变量值
  PID->value_err[0] = PID->target_value - PID->present_value; // 更新本次偏差
  // if(fabs((PID->value_err[0]) < 2)&&(PID==&Speed_left||PID==&Speed_right))
  // PID->value_err[0] = 0;//pid死区
  PID->PID_out_P = (PID->PID_para->kp) * (PID->kp_ratio) *
                   (PID->value_err[0] - PID->value_err[1]); // 计算PID中的KP分量
  PID->PID_out_I = (PID->PID_para->ki) * (PID->ki_ratio) *
                   (PID->value_err[0]); // 计算PID中的KI分量
  PID->PID_out_D = (PID->PID_para->kd) * (PID->kd_ratio) *
                   (PID->value_err[0] - 2 * PID->value_err[1] +
                    PID->value_err[2]); // 计算PID中的KD分量

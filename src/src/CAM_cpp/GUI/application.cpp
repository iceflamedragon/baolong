...
void GUI_Init()                // GUI初始化函数,放在while循环之前
{
  ...
  //    TSUI_FlashSave();
  //    //保存所有在调参列表上的参数，首次烧录或改变参数列表时，此时flash无数据时使用此函数，烧完之后记得注释掉这行再重新烧录一遍
...
struct paralist_s paralist[100] = // 可调参数列表
    {
        // 格式： {参数w地址,参数名称,调节精度},
        // 注：1.参数只能是整形参数 2.参数名称只能是字符串且最多16个字符 3.调节精度最小为1
        // // 4.受到多种原因的限制，最多只能同时调100个参数（序号0—99）
...
struct paralist_s paralist2[100] = // 可调参数列表
    {
        ...
        {0}};
void set_setpara(float turn_PIDkp, float turn_PIDkd, float gyroturn_PIDkp,
...
void init_setpara() // 各个参数的初始化，不在参数表上的参数仍然可以正常初始化与使用，只是不可调
{
  ...
  setpara.obstacle_speed =  1.6;
  setpara.broken_target_speed =  1.6;
  setpara.differ_ratio = 55;
  setpara.loop_target_speed = 2; /// 原先为48
  setpara.big_loop_speed =  1.6;   // 大环内的目标速度没用上
  setpara.slope_speed =  1.6;
  setpara.cross_speed =  1.6;
  setpara.fuzzy_kp = 90;

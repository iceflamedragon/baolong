  ...
  bool process(Tracking &track, vector<PredictResult> predicts) {
    ...
    case TypeRace::Safe: // 普通车辆
    {
      ...
      if (predict.x > 0 && predict.y > 0) // 检测到有效的AI标志
      {
        ...
        if (row < 0) {
          _index = 1;
          if (abs(track.pointsEdgeLeft[0].y - predict.x) >
              abs(track.pointsEdgeRight[0].y - predict.x - predict.width))
            sideLeft = true;
        } else if (row >= track.pointsEdgeLeft.size() ||
                   row >= track.pointsEdgeRight.size()) {
          _index = 2;
          if (abs(track.pointsEdgeLeft[track.pointsEdgeLeft.size() - 1].y -
                  predict.x) >
              abs(track.pointsEdgeRight[track.pointsEdgeRight.size() - 1].y -
                  predict.x - predict.width))
            sideLeft = true;
        } else {
          _index = 3;
          if (abs(track.pointsEdgeLeft[row].y - predict.x) >
              abs(track.pointsEdgeRight[row].y - predict.x - predict.width))
            sideLeft = true;
        }
  ...
  PredictResult searchSign(vector<PredictResult> predicts, int index) {
    ...
    for (int i = 0; i < predicts.size(); i++) {
      if (predicts[i].type == index) {
        // 通过框大小过滤最佳目标
        if (predicts[i].height * predicts[i].width >
            predict.height * predict.width) {
          predict = predicts[i];
        }
      }
    }

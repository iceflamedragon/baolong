  ...
  bool process(Tracking &track, vector<PredictResult> predict, Motion &motion) {
    ...
    case Step::Enable: //[02] 使能
    {
      ...
      if (entryLeft)                // 左入库
      {
        ...
        if (((_pointNearConeL.x > ROWSIMAGE * 0.6) ||
      ...
      } else // 右入库
      {
        ...
        if (((_pointNearConeL.x > ROWSIMAGE * 0.6) ||
             (_pointNearConeL.x > ROWSIMAGE * 0.6)) &&
            !flagin) { // 当车辆开始靠近右边锥桶：准备入库
          reflag = 1;
          flagtrace = 1;
          cout << "锥桶到达0.6的位置了" << endl << endl << endl;
...
private:
  ...
  void searchCones(vector<PredictResult> predict, Motion &motion) {
    ...
    for (int i = 0; i < predict.size(); i++) {
      ...
      {
        if (predict[i].height * predict[i].width > motion.params.areaMax)
          continue; ////////如果锥桶面积大于一定值，就排除他

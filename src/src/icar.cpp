...
public:
  WorkerThread(MainWindow *window) : m_window(window) {}
  ...
  void run() override {
    ...
    while (1) {
      ...
      if (motion.params.debug) // 开启视频
      {
        ...
        float *params_mode =
            set_show_params_mode(motion.params.show_params_mode);
      ...
      // 更新参数显示
      QString params = QString("目标速度: %1, 舵机PWM: %2")
                           .arg(mycar.uart_speed)
                           .arg(mycar.uart_servo);
...
int main(int argc, char *argv[]) {
  ...
  WorkerThread workerThread(&window);
  QObject::connect(&workerThread, &WorkerThread::updateParams, &window,
                   &MainWindow::updateParams);
  workerThread.start();

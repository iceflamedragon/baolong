...
public: // int leftpoint;
  // int rightpoint;
  int center_flag_right=0;
  int center_flag_left=0;
  int flag_closeai;
  int flagbigringl = 0;
  int flagbigringr = 0;
  int flagpid = 0;
...
#define RingRight 2
  ...
  bool process(Tracking &track, Mat &imagePath, Motion &motion) {
    ...
    if ((ringStep == RingStep::Entering)) /// 有
    {
      ...
      left_breakpoint = Find_Left_Breakpoint(track, 20, 180);
    ...
    // 出环补线
    if (ringStep == RingStep::Inside) {
      // if (abs(mpu6050_now - mpu6050_later) > 100 &&
      //     abs(mpu6050_now - mpu6050_later) < 220) // 找固定打角
      // {
      //   center_sum_flag = Center_Sum_Start;
      // } else
      cout<<"角度积分差值"<<abs(mpu6050_now- mpu6050_later)<<endl;
       if (abs(mpu6050_now - mpu6050_later) > 220&&abs(mpu6050_now - mpu6050_later) <=260) {
        // center_sum_flag = Center_Sum_End;

        if (ringType == RingLeft) {
          int breakpoint_out;

          breakpoint_out = Monotonicity_Change_Right(track, 50, 180); // 找到拐点
          if (breakpoint_out < 180 && breakpoint_out > 50)

          {
            K_Add_Boundry_Right(
                regression(track.pointsEdgeRight, breakpoint_out - 20,
                           breakpoint_out - 2),
                track.pointsEdgeRight[breakpoint_out - 2].y, breakpoint_out - 2,
                200, track); // 根据斜率作直线
          }
        }
        if (ringType == RingRight) {
          ...
          breakpoint_out = Monotonicity_Change_Left(track, 50, 180); // 找到拐点
          if (breakpoint_out < 180 && breakpoint_out > 50)

          {
            K_Add_Boundry_Left(
                regression(track.pointsEdgeLeft, breakpoint_out - 20,
                           breakpoint_out - 2),
                track.pointsEdgeLeft[breakpoint_out - 2].y, breakpoint_out - 2,
                200, track); // 根据斜率作直线
          }
        }
      }
      ...
      //     //   cout << "下半部分  " << slopeButtom << "   上半部分" <<
      //     slopeTop
      //     //        << endl
      //     //        << endl;
      //     //   rowBreakpointLeft =
      //     track.pointsEdgeRight[track.validRowsLeft].x;
      ...
      //   //   POINT p_start(max(rowBreakpointRight, ROWSIMAGE - 80),
      //   COLSIMAGE);
      //   //   POINT p_mid((ROWSIMAGE - 50 + rowBreakpointLeft) / 4, COLSIMAGE
      //   / 2);
      ...
      //   //     slopeTop =
      //   (track.pointsEdgeLeft[rowBreakLeft].y-track.pointsEdgeLeft[rowBreakLeft
      //   + 20].y
      //   //                 ) *
      //   //                100 /
      //   //                (track.pointsEdgeLeft[rowBreakLeft + 20].x -
      ...
      //   //          << endl;
      //   //     rowBreakpointRight =
      //   track.pointsEdgeLeft[track.validRowsRight].x;

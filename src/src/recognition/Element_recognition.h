...
struct watch_o {
  ...
  int RLStraight; // 左右直线 0：无直线 1左边线是直线，右边线不是直线
                  // 2：右边线是直线左边线不是 3：左右边线都是直线
  ...
  // 直线检测相关
  int16 StrNumL; // 直线检测算法所得到的的左侧小段直线段的数目 使用位置：void
                 // Straight_check(void);
  int16 StrNumR; // 直线检测算法所得到的的右侧小段直线段的数目
  ...
  int16 distanceThres; // 积分阈值，路程积分到此值时结束
  char Yaw_flag; // 角度积分标志 0：未开始积分 1：正在进行角度积分
                 // 2：角度积分完成
  ...
  uint8 out_garage_flag; // 出库标记位 1：左出库前直行 2：向左打角出库
                         // 3：右出库前直行 4：向右打角出库 0:出库结束，正常行驶
  // 停车标记位
  uint8 garage_stop;
  uint8 garage_flag;
  // 误差计算相关

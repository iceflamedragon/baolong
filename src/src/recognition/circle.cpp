...
void left_ring_first_angle() {
  if (watch.InLoop != 0 && watch.InLoop != 1)
    return; // 在循环之前跳出，节省时间
  for (int y = loop_forward_near; y < loop_forward_far; y++) // 逐行扫描
  {
    if ( // lineinfo[y + 3].left_lost
        ...
        !lineinfo[y - 2].left_lost && !lineinfo[y - 4].left_lost &&
        !lineinfo[y].left_lost && !lineinfo[y + 1].right_lost &&
        !lineinfo[y].right_lost && !lineinfo[y - 1].right_lost &&
      ...
      if (Element == None) // 在当前无元素时进行以下操作，其他时候只找角点
      {
        ...
        // Element=Left_ring_confirm;        //进入左环二次确认
        left_ring_confirm();

        // watch.InLoop = 1;
        // watch.InLoopAngle = 0;
        // mycar.target_speed=setpara.loop_target_speed;//将小车当前目标速度设为环内速度
      }
      break;
    }
  }
...
void left_ring_confirm() {
  uint8 zebra_confirm = 0, white_count1 = 0, white_count2 = 0, white_count3 = 0,
        black_count, right_lost = 0;
  // right_ring_first_angle();//扫描是否存在右环角点
  // left_ring_circular_arc();//扫描是否存在左环上弧
  for (int y = loop_forward_near; y < loop_forward_far; y++) // 逐行扫描
  {
    if ((lineinfo[y + 2].right - lineinfo[y].right) > 3)
      right_lost = 1;
  }
  if (right_lost == 0) {
    ...
    if ((white_count1 >= 5 && white_count2 >= 5 && white_count3 >= 5) ||
        (black_count < 3)) {
      ...
      if (Element_rem.loop_data[Element_rem.loop_count] == 0) // 如果是小环
      {
        ...
        change_pid_para(&CAM_Turn,
                        &setpara.loop_turn_PID); // 将转向PID参数调为环内转向PID
      } else                                     // 如果是大环
      {
...
void left_ring_circular_arc() {
  if (watch.InLoop != 1 && watch.InLoop != 2)
    return; // 在循环之前跳出，节省时间
  // beep(20);
  for (int y = loop_forward_near; y < loop_forward_far; y++) // 逐行扫描
  {
    if ( // y <watch.InLoopAngle2  &&
        y > watch.InLoopAngleL && y < watch.InLoopCirc &&
...
void left_ring_second_angle() {
  ...
  for (int y = loop_forward_near; y < loop_forward_far; y++) // 逐行扫描
  {
    if (watch.InLoopCirc < 120 && y < watch.InLoopAngle2 &&
        watch.InLoopAngle2 == 120 &&
        get_integeral_state(&distance_integral) == 2 && y > 60 &&
        y < (loop_forward_far - 2) && y > watch.InLoopCirc &&
        lineinfo[watch.InLoopCirc].left_lost && lineinfo[y + 1].left > 30 &&
  ...
  if (watch.InLoopAngle2 != 120 && watch.InLoopAngle2 > 50) {
    find_angle_left_down(watch.InLoopAngle2_x, watch.InLoopAngle2,
                         &watch.InLoopAngle2_x, &watch.InLoopAngle2);
  }
}
...
void left_ring_begin_turn() {
  ...
  if (get_integeral_state(&distance_integral) == 2 // 路程积分完成
      && watch.InLoop == 1 && lineinfo[watch.InLoopCirc].left_lost &&
      watch.InLoopAngle2 <= 90) {
...
void left_ring_in_loop() {
  if (watch.InLoop != 2)
    return;
  if (watch.InLoop == 2 && get_integeral_state(&angle_integral) == 1 &&
...
void left_ring_prepare_out() {
  if (watch.InLoop != 3)
    return;
  if (watch.InLoop == 3 && get_integeral_state(&angle_integral) == 1 &&
      get_integeral_data(&angle_integral) > 160) {
    watch.InLoop = 4;
    watch.OutLoop_turn_point_x = lineinfo[79].right;
    beep2(4, 20);
  }
}
// 检测出环时右角点位置
...
void left_ring_out_loop_turn() {
  ...
  if (watch.InLoop == 4
    ...
    begin_distant_integeral(2000); // 开启路程积分，此时要保持左转
    watch.OutLoop = 1;
    beep2(5, 20);
  }
}
...
void left_ring_out_loop() {
  ...
  for (int y = loop_forward_near; y < loop_forward_far; y++) // 逐行扫描
  {
    if ((watch.InLoop == 4 && watch.OutLoop == 1 && y < 60 &&
         lineinfo[y].right_lost == 0 && lineinfo[y + 10].right_lost == 0 &&
         (188 - (lineinfo[y].right - lineinfo[y + 10].right) * (115 - y) / 10) >
             40
...
void left_ring_straight_out_angle() {
  ...
  for (int y = loop_forward_near; y < loop_forward_far; y++) // 逐行扫描
  {
    if ((watch.InLoop == 5) && watch.OutLoop == 1 &&
        ...
        (lineinfo[y + 2].right - lineinfo[y + 2].left) <
            (lineinfo[y - 1].right - lineinfo[y - 1].left) - 30 &&
        (lineinfo[y + 1].right - lineinfo[y + 1].left) <
            (lineinfo[y - 2].right - lineinfo[y - 2].left) - 30 &&
        (lineinfo[y].right - lineinfo[y].left) <
            (lineinfo[y - 3].right - lineinfo[y - 3].left) - 30) {
...
void left_ring_complete_out() {
  ...
  for (int y = loop_forward_near; y < loop_forward_far; y++) // 逐行扫描
  {
    if (watch.InLoop == 5 && get_integeral_state(&distance_integral) == 2 &&
        watch.OutLoopAngle2 < 60) {
...
void right_ring_first_angle() {
  ...
  for (int y = loop_forward_near; y < loop_forward_far; y++) // 逐行扫描
  {
    if (y < 86
        ...
        y < watch.InLoopAngleR && lineinfo[y + 4].right - lineinfo[y].right > 10
...
void right_ring_confirm() {
  uint8 zebra_confirm, white_count1 = 0, white_count2 = 0, white_count3 = 0,
                       left_lost = 0, black_count = 0;
  // right_ring_circular_arc();  //扫描是否存在右环上弧
  // left_ring_first_angle();   //扫描是否存在左环角点
  for (int y = loop_forward_near; y < loop_forward_far; y++) // 逐行扫描
  {
    if ((lineinfo[y].left - lineinfo[y + 2].left) > 3)
      left_lost = 1;
  }
  if (left_lost == 0) {
    {
...
void right_ring_begin_turn() {
  if (watch.InLoop != 6)
    return;
  if (get_integeral_state(&distance_integral) == 2 // 路程积分完成
      && watch.InLoop == 6 && (watch.InLoopAngle2 <= 90) &&
...
void right_ring_circular_arc() {
  ...
  for (int y = loop_forward_near; y < loop_forward_far; y++) // 逐行扫描
  {
    if ((watch.InLoop == 6) && y < watch.InLoopAngle2 &&
        y > watch.InLoopAngleR && y < watch.InLoopCirc &&
        !lineinfo[y + 2].right_lost && !lineinfo[y + 1].right_lost &&
        !lineinfo[y - 2].right_lost && !lineinfo[y - 1].right_lost &&
        lineinfo[y + 1].right >= lineinfo[y].right &&
        lineinfo[y + 2].right >= lineinfo[y + 1].right &&
        lineinfo[y - 1].right >= lineinfo[y].right &&
        lineinfo[y - 2].right >= lineinfo[y - 1].right) { // 入环点所在行
      watch.InLoopCirc = y;
      break;
    }
  }
...
void right_ring_second_angle() {
  if (watch.InLoop != 6 && watch.InLoop != 7)
    return;                      // 在循环之前跳出，节省时间
  for (int y = 79; y < 100; y++) // 最靠下的点，近距离时较为靠谱（第一判据）
  {
    /*        if(watch.InLoopCirc<120
                   &&y<watch.InLoopAngle2
                   &&get_integeral_state(&distance_integral)==2
  ...
  for (int y = loop_forward_near; y < loop_forward_far; y++) // 逐行扫描
  {
    if (watch.InLoopCirc < 120 && y < watch.InLoopAngle2 &&
        watch.InLoopAngle2 == 120 &&
        get_integeral_state(&distance_integral) == 2 && y > 60 &&
        lineinfo[watch.InLoopCirc].right_lost && y < watch.InLoopAngle2 &&
        y > watch.InLoopCirc &&
        (lineinfo[y].right - lineinfo[y + 1].right) <= 2 &&
        (lineinfo[y - 4].right - lineinfo[y].right) >
            (188 - lineinfo[y].right) / 2) {
      watch.InLoopAngle2 = y;
...
void right_ring_in_loop() {
  if (watch.InLoop == 7 && get_integeral_state(&angle_integral) == 1 &&
      get_integeral_data(&angle_integral) < -40 // 转过一定角度
...
void right_ring_prepare_out() {
  if (watch.InLoop == 8 && get_integeral_state(&angle_integral) == 1 &&
      get_integeral_data(&angle_integral) < -160) {
    watch.InLoop = 9;
    watch.OutLoop_turn_point_x = lineinfo[79].left;
    beep2(4, 20);
  }
}
//
...
void right_ring_out_loop_turn() {
  if (watch.InLoop == 9
      ...
      && watch.OutLoopAngle1 < 120 &&
      get_integeral_state(&angle_integral) == 2 &&
      lineinfo[watch.OutLoopAngle1].left_lost == 1 && watch.OutLoop == 0) {
    if (Element_rem.loop_data[Element_rem.loop_count] == 0) // 如果是小环
    {
      begin_distant_integeral(-setpara.loop_out_distance);
    } else // 如果是大环
...
void right_ring_out_loop() {
  if (watch.InLoop != 9 && watch.OutLoop != 1)
    return;
  for (int y = loop_forward_near; y < loop_forward_far; y++) // 逐行扫描
  {
    if ((watch.InLoop == 9 && watch.OutLoop == 1 && y < 60 &&
         lineinfo[y].left_lost == 0 && lineinfo[y + 10].left_lost == 0 &&
...
void right_ring_complete_out() {
  if (watch.InLoop == 10 && get_integeral_state(&distance_integral) == 2 &&
      watch.OutLoopAngle2 < 60) {
    // clear_all_flags();// 出环成功,清除所有标志
    out_element();
    // mycar.target_speed=setpara.com_target_speed;//恢复正常速度
